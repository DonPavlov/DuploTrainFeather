...
void Lpf2Hub::parseSensorMessage(uint8_t *pData)
{
  ...
  else if ((deviceType == (byte)DeviceType::MEDIUM_LINEAR_MOTOR) ||
           (deviceType == (byte)DeviceType::MOVE_HUB_MEDIUM_LINEAR_MOTOR))
  {
...
/**
 * @brief Init function set the UUIDs and scan for the Hub
 * @param [in] deviceAddress to which the arduino should connect represented by a hex string of the format:
 * 00:00:00:00:00:00
 */
...
int Lpf2Hub::getDeviceIndexForPortNumber(byte portNumber)
{
  ...
  {
    log_v("device %d, port number: %x, device type: %x, callback address: %x",
          idx,
          connectedDevices[idx].PortNumber,
          connectedDevices[idx].DeviceType,
          connectedDevices[idx].Callback);
 ...
 */
void Lpf2Hub::activateHubPropertyUpdate(HubPropertyReference      hubProperty,
                                        HubPropertyChangeCallback hubPropertyChangeCallback)
{
 ...
 * @param [in] optional callback function which will be called if a value has changed
 */
void Lpf2Hub::requestHubPropertyUpdate(HubPropertyReference      hubProperty,
                                       HubPropertyChangeCallback hubPropertyChangeCallback)
{
  if (hubPropertyChangeCallback != nullptr)
  {
...
void Lpf2Hub::setTachoMotorSpeed(byte port, int speed, byte maxPower, BrakingStyle brakingStyle)
{
  ...
  byte setMotorCommand[8] =
  { 0x81, port, 0x11, 0x01, LegoinoCommon::MapSpeed(speed), maxPower, (byte)brakingStyle, 0x03 };
...
void Lpf2Hub::setTachoMotorSpeedForTime(byte port, int speed, int16_t time = 0, byte maxPower, BrakingStyle brakingStyle)
{
  // Use acc and dec profile (0x03 last two bits set)
  byte *timeBytes           = LegoinoCommon::Int16ToByteArray(time);
  byte  setMotorCommand[10] =
  { 0x81, port, 0x11, 0x09, timeBytes[0], timeBytes[1], LegoinoCommon::MapSpeed(speed), maxPower, (byte)brakingStyle,
    0x03 };
 ...
 * @param [in] brakingStyle Braking style how the motor will stop. Brake(default), Float, Hold are available
 */
void Lpf2Hub::setTachoMotorSpeedForDegrees(byte port, int speed, int32_t degrees, byte maxPower,
                                           BrakingStyle brakingStyle)
{
  byte *degreeBytes = LegoinoCommon::Int32ToByteArray(degrees);

  // Use acc and dec profile (0x03 last two bits set)
  byte setMotorCommand[12] =
  { 0x81,                           port,                           0x11,                                     0x0B,
    degreeBytes[0],                 degreeBytes[1],                 degreeBytes[2],
    degreeBytes[3],
    LegoinoCommon::MapSpeed(speed), maxPower,                       (byte)brakingStyle,                       0x03 };
 ...
 * @param [in] position Position in degrees (relative to zero point on power up, or encoder reset)
...
void Lpf2Hub::setAbsoluteMotorPosition(byte port, int speed, int32_t position, byte maxPower, BrakingStyle brakingStyle)
{
  ...
  byte setMotorCommand[12] =
  { 0x81,                           port,                           0x11,                                     0x0D,
    positionBytes[0],               positionBytes[1],
    positionBytes[2],               positionBytes[3],
    LegoinoCommon::MapSpeed(speed), maxPower,                       (byte)brakingStyle,                       0x03 };
...
void Lpf2Hub::setAbsoluteMotorEncoderPosition(byte port, int32_t position)
{
  ...
  byte setMotorCommand[9] =
  { 0x81, port, 0x11, 0x51, 0x02, positionBytes[0], positionBytes[1], positionBytes[2], positionBytes[3] };
 ...
 * @param [in] brakingStyle Braking style how the motor will stop. Brake(default), Float, Hold are available
 */
void Lpf2Hub::setTachoMotorSpeedsForDegrees(int          speedLeft,
                                            int          speedRight,
                                            int32_t      degrees,
                                            byte         maxPower,
                                            BrakingStyle brakingStyle)
{
  ...
  // Use acc and dec profile (0x03 last two bits set)
  byte setMotorCommand[13] =
  { 0x81,           port,                                  0x11,                                  0x0C,
    degreeBytes[0], degreeBytes[1],
    degreeBytes[2], degreeBytes[3],                        LegoinoCommon::MapSpeed(
      speedLeft),   LegoinoCommon::MapSpeed(speedRight),   maxPower,                              (byte)brakingStyle,
    0x03 }; // boost with time
